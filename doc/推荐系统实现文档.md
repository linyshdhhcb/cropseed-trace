# 推荐系统实现文档

## 1. 系统概述

### 1.1 架构设计
本推荐系统采用**混合推荐架构**，实现了真正的多算法融合策略：
- **协同过滤推荐** (30%权重) - 基于用户相似度的协同过滤
- **内容推荐** (25%权重) - 基于商品特征相似性推荐
- **个性化推荐** (25%权重) - 基于用户画像的个性化推荐
- **热门推荐** (20%权重) - 基于市场热度的推荐

### 1.2 技术实现核心

####  **算法逻辑**
- **真正的混合推荐**: 实现加权融合而非简单选择，解决核心逻辑错误
- **智能降权**: 对最近购买商品降权而非完全过滤，提高推荐多样性
- **配置化算法**: 所有算法参数可配置，便于生产环境调优

#### 🛡️ **健壮性增强**
- **参数校验**: 全面的边界条件检查
- **日志增强**: 详细的执行日志和性能监控
- **容错设计**: 单个算法失败不影响整体推荐

## 2. 数据模型与配置

### 2.1 用户行为表 (user_behavior)
```sql
CREATE TABLE user_behavior (
    id BIGINT PRIMARY KEY,
    user_id BIGINT NOT NULL,
    behavior_type INT NOT NULL COMMENT '1-浏览,2-搜索,3-收藏,4-加购物车,5-购买,6-评价',
    target_type INT DEFAULT 1 COMMENT '目标类型:1-商品',
    target_id BIGINT NOT NULL COMMENT '目标ID',
    behavior_content TEXT COMMENT '行为内容',
    behavior_weight DECIMAL(3,2) DEFAULT 1.0 COMMENT '行为权重',
    rating INT COMMENT '评分(1-5)',
    behavior_time DATETIME NOT NULL,
    session_id VARCHAR(64) COMMENT '会话ID',
    deleted_flag TINYINT DEFAULT 0
);
```

### 2.2 用户画像表 (user_profile)
```sql
CREATE TABLE user_profile (
    id BIGINT PRIMARY KEY,
    user_id BIGINT NOT NULL UNIQUE,
    user_type INT DEFAULT 1 COMMENT '用户类型:1-个人,2-企业',
    region VARCHAR(100) COMMENT '地区',
    activity_level DECIMAL(3,2) COMMENT '活跃度(0-1)',
    purchase_frequency DECIMAL(3,2) COMMENT '购买频率(0-1)',
    price_sensitivity DECIMAL(3,2) COMMENT '价格敏感度(0-1)',
    quality_requirement DECIMAL(3,2) COMMENT '质量要求(0-1)',
    brand_loyalty DECIMAL(3,2) COMMENT '品牌忠诚度(0-1)',
    purchase_preference VARCHAR(200) COMMENT '购买偏好',
    recommendation_weight DECIMAL(3,2) DEFAULT 1.0 COMMENT '推荐权重'
);
```

### 2.3 商品特征表 (seed_features)
```sql
CREATE TABLE seed_features (
    id BIGINT PRIMARY KEY,
    seed_id BIGINT NOT NULL UNIQUE,
    price_feature DECIMAL(3,2) COMMENT '价格特征',
    quality_feature DECIMAL(3,2) COMMENT '质量特征', 
    brand_feature DECIMAL(3,2) COMMENT '品牌特征',
    market_heat DECIMAL(3,2) COMMENT '市场热度',
    user_rating DECIMAL(3,2) COMMENT '用户评分',
    recommendation_weight DECIMAL(3,2) DEFAULT 1.0
);
```

## 3. 推荐算法数学公式

### 3.1 协同过滤算法数学模型

#### 3.1.1 用户相似度计算

**余弦相似度公式：**
$$
sim(u,v) = cos(θ) = (Ru · Rv) / (||Ru|| × ||Rv||)
$$
其中：
- Ru, Rv 分别表示用户u和用户v的评分向量
- Ru · Rv 表示向量点积：Σ(Rui × Rvi)
- ||Ru|| 表示向量模长：√(Σ(Rui²))

**综合相似度公式：**
$$
SimTotal(u,v) = α × SimBehavior(u,v) + β × SimProfile(u,v)
$$
其中：
- α = 0.7 (行为相似度权重)
- β = 0.3 (画像相似度权重)
- α + β = 1

**用户画像相似度：**
$$
SimProfile(u,v) = (1/n) × Σ[similarity_factor_i]

similarity_factor_i = {
    0.3, if userType(u) = userType(v)
    0.2, if region(u) = region(v)  
    0.2 × (1 - |priceSensitivity(u) - priceSensitivity(v)|)
    0.2 × (1 - |qualityRequirement(u) - qualityRequirement(v)|)
    0.1, if purchasePreference(u) = purchasePreference(v)
}
$$

#### 3.1.2 隐式评分计算

**行为评分公式：**
$$
Rating(u,i) = Σ[BaseScore(type) × Weight(behavior) × TimeDecay(t)]
$$
其中：
BaseScore(type) = {
    1.0,  type = 1 (浏览)
    1.5,  type = 2 (搜索)
    3.0,  type = 3 (收藏)
    3.5,  type = 4 (加购物车)
    5.0,  type = 5 (购买)
    4.0,  type = 6 (评价)
}

**时间衰减函数：**
$$
TimeDecay(t) = {
    1.0,  if Δt ≤ 30天
    0.8,  if 30天 < Δt ≤ 60天
    0.6,  if 60天 < Δt ≤ 90天
    0.4,  if 90天 < Δt ≤ 180天
    0.2,  if Δt > 180天
}
$$
其中 Δt = current_time - behavior_time

#### 3.1.3 协同过滤预测评分

**预测评分公式：**
$$
P(u,i) = Σ[sim(u,v) × Rating(v,i)] / Σ|sim(u,v)|
$$
其中：
- P(u,i) 表示用户u对商品i的预测评分
- sim(u,v) 表示用户u与用户v的相似度
- Rating(v,i) 表示用户v对商品i的实际评分
- 求和范围为所有与用户u相似且对商品i有行为的用户v

**降权处理：**
$$
P'(u,i) = {
    P(u,i) × 0.3,  if i ∈ RecentPurchased(u)
    P(u,i),        otherwise
}

RecentPurchased(u) = {i | 用户u在过去30天内购买过商品i}
$$


### 3.2 内容推荐算法数学模型

#### 3.2.1 商品特征向量

**商品特征表示：**
$$
Item(i) = [price_feature, quality_feature, brand_feature, market_heat, user_rating]
$$
其中每个特征值范围为 [0, 1]

**特征相似度计算：**
$$
ContentSim(i,j) = cos(Item(i), Item(j)) = (Item(i) · Item(j)) / (||Item(i)|| × ||Item(j)||)
$$

#### 3.2.2 用户偏好建模

**用户偏好向量：**
$$
UserPref(u) = Σ[Rating(u,i) × Item(i)] / Σ[Rating(u,i)]
$$
其中求和范围为用户u有行为的所有商品i

**内容推荐评分：**
$$
ContentScore(u,i) = UserPref(u) · Item(i)
$$


### 3.3 个性化推荐数学模型

#### 3.3.1 用户画像指标计算

**活跃度计算：**
$$
ActivityLevel(u) = min(RecentBehaviorCount(u) / 30, 1.0)

RecentBehaviorCount(u) = |{b | b ∈ Behaviors(u) ∧ time(b) > current_time - 30天}|
$$
**购买频率计算：**
$$
PurchaseFrequency(u) = min(PurchaseCount(u) / 10, 1.0)

PurchaseCount(u) = |{b | b ∈ Behaviors(u) ∧ type(b) = 5}|
$$
**价格敏感度计算：**
$$
PriceSensitivity(u) = max(0, min(1, (MaxPrice - AvgPurchasePrice(u)) / MaxPrice))

AvgPurchasePrice(u) = (Σ price(i)) / |PurchasedItems(u)|
$$
其中 MaxPrice = 200 (假设的最高价格)

**质量要求计算：**
$$
QualityRequirement(u) = AvgRating(u) / 5.0

AvgRating(u) = (Σ rating(u,i)) / |RatedItems(u)|
$$


#### 3.3.2 个性化评分

**基于价格的评分：**
$$
PriceScore(u,i) = {
    1.0,  if PriceSensitivity(u) > 0.7 ∧ price(i) < 0.3 × MaxPrice
    0.0,  otherwise
}
$$
**基于质量的评分：**
$$
QualityScore(u,i) = {
    quality_feature(i),  if QualityRequirement(u) > 0.7
    0.0,                 otherwise
}
$$


### 3.4 混合推荐数学模型

#### 3.4.1 加权融合公式

**混合评分计算：**
$$
HybridScore(u,i) = w₁×CF_Score(u,i) + w₂×Content_Score(u,i) + w₃×Personal_Score(u,i) + w₄×Popular_Score(i)
$$
其中权重分配：
w₁ = 0.30 (协同过滤权重)
w₂ = 0.25 (内容推荐权重)  
w₃ = 0.25 (个性化推荐权重)
w₄ = 0.20 (热门推荐权重)

约束条件：w₁ + w₂ + w₃ + w₄ = 1.0

#### 3.4.2 最终排序

**归一化处理：**
$$
NormalizedScore(u,i) = (HybridScore(u,i) - min_score) / (max_score - min_score)
$$
其中：
min_score = min{HybridScore(u,j) | j ∈ CandidateItems}
max_score = max{HybridScore(u,j) | j ∈ CandidateItems}

**Top-N推荐：**
$$
Recommendations(u,N) = TopN{(i, NormalizedScore(u,i)) | i ∈ CandidateItems}
$$
按 NormalizedScore(u,i) 降序排列，取前N个商品

### 3.5 评估指标数学公式

#### 3.5.1 准确性指标

**精确率 (Precision)：**
$$
Precision@N = |RelevantItems ∩ RecommendedItems| / |RecommendedItems|
$$
其中 N 为推荐列表长度

**召回率 (Recall)：**
$$
Recall@N = |RelevantItems ∩ RecommendedItems| / |RelevantItems|
$$
**F1分数：**
$$
F1@N = 2 × (Precision@N × Recall@N) / (Precision@N + Recall@N)
$$


#### 3.5.2 多样性指标

**推荐多样性：**
$$
Diversity = (1 / (N×(N-1))) × ΣΣ(1 - sim(i,j))
           i=1 j=i+1
$$
其中 sim(i,j) 为商品i和商品j的相似度

#### 3.5.3 覆盖率指标

**目录覆盖率：**
```java
@Override
public List<RecommendationVO> collaborativeFilteringRecommend(Long userId, Integer limit) {
    try {
        // 1. 获取用户画像和行为数据
        UserProfile userProfile = userProfileMapper.selectByUserId(userId);
        if (userProfile == null) {
            log.warn("用户画像不存在，userId: {}", userId);
            return popularRecommend(limit);
        }
        
        List<UserBehavior> userBehaviors = userBehaviorMapper.selectByUserId(userId, 200); // 增加数据量
        if (CollUtil.isEmpty(userBehaviors)) {
            log.info("用户无历史行为记录，返回热门推荐，userId: {}", userId);
            return popularRecommend(limit);
        }
        
        // 2. 批量获取候选用户（解决N+1查询问题）
        List<UserProfile> candidateProfiles = userProfileMapper.selectSimilarProfiles(
                userProfile.getUserType(), userProfile.getRegion(), 30);
        
        List<Long> candidateUserIds = candidateProfiles.stream()
                .map(UserProfile::getUserId)
                .filter(id -> !id.equals(userId))
                .collect(Collectors.toList());
        
        Map<Long, List<UserBehavior>> candidateBehaviorsMap = batchGetUserBehaviors(candidateUserIds);
        
        // 3. 优化的用户相似度计算
        Map<Long, Double> userSimilarities = calculateUserSimilaritiesOptimized(
                userId, userBehaviors, candidateBehaviorsMap, candidateProfiles);
        
        // 4. 筛选高相似度用户（使用配置阈值）
        List<Long> similarUserIds = userSimilarities.entrySet().stream()
                .filter(entry -> entry.getValue() > recommendationConfig.getSimilarityThreshold())
                .sorted(Map.Entry.<Long, Double>comparingByValue().reversed())
                .limit(15) // 增加候选用户数量
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
        
        // 5. 计算商品推荐评分（智能降权）
        Map<Long, Double> itemScores = calculateItemScores(
                userBehaviors, candidateBehaviorsMap, similarUserIds, userSimilarities);
        
        // 6. 过滤已购买商品并返回结果
        Set<Long> userPurchasedItems = getUserPurchasedItems(userBehaviors);
        return itemScores.entrySet().stream()
                .filter(entry -> !userPurchasedItems.contains(entry.getKey()))
                .sorted(Map.Entry.<Long, Double>comparingByValue().reversed())
                .limit(limit)
                .map(this::createRecommendationVO)
                .collect(Collectors.toList());
                
    } catch (Exception e) {
        log.error("协同过滤推荐失败，userId: {}", userId, e);
        return Collections.emptyList();
    }
}
```

#### 4.1.2 批量查询优化（解决N+1问题）
```java
private Map<Long, List<UserBehavior>> batchGetUserBehaviors(List<Long> userIds) {
    if (CollUtil.isEmpty(userIds)) return new HashMap<>();
    
    Map<Long, List<UserBehavior>> result = new HashMap<>();
    int batchSize = 100;
    
    for (int i = 0; i < userIds.size(); i += batchSize) {
        List<Long> batchIds = userIds.subList(i, Math.min(i + batchSize, userIds.size()));
        
        // 批量查询而非逐个查询
        List<UserBehavior> behaviors = userBehaviorMapper.selectByUserIds(batchIds, 50);
        
        // 按用户ID分组
        Map<Long, List<UserBehavior>> batchResult = behaviors.stream()
                .collect(Collectors.groupingBy(UserBehavior::getUserId));
        result.putAll(batchResult);
    }
    return result;
}
```

#### 4.2.2 配置化的时间衰减机制
```java
private double calculateTimeDecay(LocalDateTime behaviorTime) {
    if (behaviorTime == null) return 0.2;
    
    long daysDiff = Duration.between(behaviorTime, LocalDateTime.now()).toDays();
    
    if (daysDiff <= 30) return 1.0;      // 30天内权重为1
    else if (daysDiff <= 60) return 0.8;  // 30-60天权重0.8
    else if (daysDiff <= 90) return 0.6;  // 60-90天权重0.6
    else if (daysDiff <= 180) return 0.4; // 90-180天权重0.4
    else return 0.2;                      // 180天以上权重0.2
}
```

### 4.2 行为评分机制

#### 4.2.1 配置化的隐式评分计算
```java
private Map<Long, Double> getUserItemRatings(List<UserBehavior> behaviors) {
    Map<Long, Double> ratings = new HashMap<>();
    
    for (UserBehavior behavior : behaviors) {
        Long itemId = behavior.getTargetId();
        
        // 使用配置的行为权重（关键改进）
        double baseScore = recommendationConfig.getBehaviorWeight().getWeightByType(behavior.getBehaviorType());
        
        // 对于评价行为，使用实际评分
        if (behavior.getBehaviorType() == 6 && behavior.getRating() != null) {
            baseScore = behavior.getRating().doubleValue();
        }
        
        // 考虑行为权重和时间衰减
        Double behaviorWeight = behavior.getBehaviorWeight() != null ? behavior.getBehaviorWeight() : 1.0;
        double timeDecay = calculateTimeDecay(behavior.getBehaviorTime());
        
        double finalRating = baseScore * behaviorWeight * timeDecay;
        ratings.merge(itemId, finalRating, Double::sum);
    }
    return ratings;
}
```

#### 4.2.2 智能降权机制
```java
private Map<Long, Double> calculateItemScores(
        List<UserBehavior> userBehaviors, Map<Long, List<UserBehavior>> candidateBehaviorsMap,
        List<Long> similarUserIds, Map<Long, Double> userSimilarities) {
    
    Map<Long, Double> itemScores = new HashMap<>();
    
    for (Long similarUserId : similarUserIds) {
        List<UserBehavior> similarUserBehaviors = candidateBehaviorsMap.get(similarUserId);
    else if (daysDiff <= 90) return 0.6;  // 60-90天权重0.6
    else if (daysDiff <= 180) return 0.4; // 90-180天权重0.4
    else return 0.2;                      // 180天以上权重0.2
}
```

### 4.3 混合推荐算法（v2.0 核心重构）

#### 4.3.1 真正的加权融合算法
```java
@Override
public List<RecommendationVO> hybridRecommend(Long userId, Integer limit) {
    try {
        log.info("开始混合推荐，userId: {}, limit: {}", userId, limit);
        
        // 参数校验
        if (userId == null || limit == null || limit <= 0) {
            log.warn("混合推荐参数无效，userId: {}, limit: {}", userId, limit);
            return Collections.emptyList();
        }

        // 1. 并行获取各类推荐结果（性能优化）
        CompletableFuture<List<RecommendationVO>> cfFuture = CompletableFuture.supplyAsync(() -> 
            collaborativeFilteringRecommend(userId, Math.max(10, limit / 2)));
        
        CompletableFuture<List<RecommendationVO>> cbFuture = CompletableFuture.supplyAsync(() -> 
            contentBasedRecommend(userId, Math.max(10, limit / 2)));
        
        CompletableFuture<List<RecommendationVO>> personalizedFuture = CompletableFuture.supplyAsync(() -> 
            personalizedRecommend(userId, Math.max(10, limit / 2)));
        
        CompletableFuture<List<RecommendationVO>> popularFuture = CompletableFuture.supplyAsync(() -> 
            popularRecommend(Math.max(10, limit / 2)));

        // 2. 安全获取推荐结果（带异常处理）
        List<RecommendationVO> cfRecommendations = safeGetRecommendations(cfFuture, "协同过滤");
        List<RecommendationVO> cbRecommendations = safeGetRecommendations(cbFuture, "内容推荐");
        List<RecommendationVO> personalizedRecommendations = safeGetRecommendations(personalizedFuture, "个性化推荐");
        List<RecommendationVO> popularRecommendations = safeGetRecommendations(popularFuture, "热门推荐");
        
        // 3. 真正的混合推荐算法（加权融合）
        Map<Long, HybridRecommendationItem> hybridScores = new HashMap<>();
        
        // 按照权重积累各算法的评分
        processAlgorithmResults(hybridScores, cfRecommendations, 
            recommendationConfig.getHybridWeight().getCollaborative(), "协同过滤");
        processAlgorithmResults(hybridScores, cbRecommendations, 
            recommendationConfig.getHybridWeight().getContent(), "内容推荐");
        processAlgorithmResults(hybridScores, personalizedRecommendations, 
            recommendationConfig.getHybridWeight().getPersonalized(), "个性化推荐");
        processAlgorithmResults(hybridScores, popularRecommendations, 
            recommendationConfig.getHybridWeight().getPopular(), "热门推荐");
        
        // 4. 按照最终评分排序并返回结果
        if (hybridScores.isEmpty()) {
            log.warn("所有推荐算法都没有结果，使用备选热门推荐，userId: {}", userId);
            return popularRecommend(limit);
        }
        
        List<RecommendationVO> finalResults = hybridScores.values().stream()
                .filter(item -> item.getFinalScore() > 0)
                .sorted((a, b) -> Double.compare(b.getFinalScore(), a.getFinalScore()))
                .limit(limit)
                .map(item -> {
                    RecommendationVO vo = item.getRecommendationVO();
                    vo.setRecommendationScore(BigDecimal.valueOf(item.getFinalScore()));
                    vo.setRecommendationType(99); // 混合推荐类型
                    vo.setRecommendationTypeName("混合推荐");
                    vo.setRecommendationReason("混合推荐:" + item.getReason());
                    return vo;
                })
                .collect(Collectors.toList());
                
        // 5. 如果结果不足，补充热门推荐
        if (finalResults.size() < limit) {
            int remaining = limit - finalResults.size();
            List<RecommendationVO> supplementary = popularRecommend(remaining * 2);
            Set<Long> existingIds = finalResults.stream()
                .map(RecommendationVO::getTargetId).collect(Collectors.toSet());
            
            supplementary.stream()
                .filter(vo -> !existingIds.contains(vo.getTargetId()))
                .limit(remaining)
                .forEach(finalResults::add);
        }
        
        return finalResults;
        
    } catch (Exception e) {
        log.error("混合推荐失败，降级为热门推荐，userId: {}, limit: {}", userId, limit, e);
        // 异常情况下返回热门推荐作为备选方案
        try {
            return popularRecommend(limit);
        } catch (Exception fallbackException) {
            log.error("备选热门推荐也失败，userId: {}", userId, fallbackException);
            return Collections.emptyList();
        }
    }
}
```

#### 4.3.2 混合推荐项目类
```java
public static class HybridRecommendationItem {
    private RecommendationVO recommendationVO;
    private double finalScore;
    private List<String> algorithms;
    private String reason;
    
    public HybridRecommendationItem(RecommendationVO vo) {
        this.recommendationVO = vo;
        this.finalScore = 0.0;
        this.algorithms = new ArrayList<>();
        this.reason = "";
    }
    
    public void addAlgorithmScore(String algorithm, double weight, double score) {
        finalScore += weight * score; // 真正的加权融合
        algorithms.add(algorithm);
        if (reason.isEmpty()) {
            reason = algorithm;
        } else {
            reason += "+" + algorithm;
        }
    }
    
    // getters...
}
```

#### 4.3.3 算法结果处理
```java
private void processAlgorithmResults(Map<Long, HybridRecommendationItem> hybridScores, 
                                    List<RecommendationVO> results, 
                                    Double weight, 
                                    String algorithmName) {
    for (RecommendationVO vo : results) {
        Long targetId = vo.getTargetId();
        double score = vo.getRecommendationScore() != null ? 
            vo.getRecommendationScore().doubleValue() : 0.0;
        
        HybridRecommendationItem item = hybridScores.computeIfAbsent(targetId, 
            k -> new HybridRecommendationItem(vo));
        
        item.addAlgorithmScore(algorithmName, weight, score);
    }
}
```

#### 4.3.4 安全获取推荐结果
```java
private List<RecommendationVO> safeGetRecommendations(
        CompletableFuture<List<RecommendationVO>> future, String algorithmName) {
    try {
        List<RecommendationVO> result = future.join();
        return result != null ? result : Collections.emptyList();
    } catch (Exception e) {
        log.warn("{}算法执行异常，跳过该算法", algorithmName, e);
        return Collections.emptyList();
    }
}
```

## 5. 配置化管理

### 5.1 推荐参数配置
```yaml
recommendation:
  # 相似度阈值
  similarity-threshold: 0.1
  # 时间衰减天数
  time-decay-days: 30
  # 最近购买天数
  recent-purchase-days: 30
  # 行为权重配置
  behavior-weight:
    view: 1.0
    search: 1.5
    collect: 3.0
    cart: 3.5
    purchase: 5.0
    rate: 4.0
  # 混合推荐权重配置
  hybrid-weight:
    collaborative: 0.30
    content: 0.25
    personalized: 0.25
    popular: 0.20
  # 用户相似度权重配置
  similarity-weight:
    behavior: 0.7
    profile: 0.3
  # 降权配置
  downgrade:
    enabled: true
    recent-purchase-ratio: 0.3
```

### 5.2 配置类实现
```java
@Data
@Configuration
@ConfigurationProperties(prefix = "recommendation")
public class RecommendationConfig {
    private Double similarityThreshold = 0.1;
    private Integer timeDecayDays = 30;
    private Integer recentPurchaseDays = 30;
    private BehaviorWeight behaviorWeight = new BehaviorWeight();
    private HybridWeight hybridWeight = new HybridWeight();
    private SimilarityWeight similarityWeight = new SimilarityWeight();
    private DowngradeConfig downgrade = new DowngradeConfig();
    
    @Data
    public static class BehaviorWeight {
        private Double view = 1.0;
        private Double search = 1.5;
        private Double collect = 3.0;
        private Double cart = 3.5;
        private Double purchase = 5.0;
        private Double rate = 4.0;
        
        public Double getWeightByType(Integer behaviorType) {
            switch (behaviorType) {
                case 1: return view;
                case 2: return search;
                case 3: return collect;
                case 4: return cart;
                case 5: return purchase;
                case 6: return rate;
                default: return 1.0;
            }
        }
    }
}
```

## 6. 用户画像系统

### 4.1 画像更新机制
```java
@Transactional(rollbackFor = Exception.class)
public void updateUserProfile(Long userId) {
    // 1. 获取用户行为统计
    List<UserBehavior> behaviors = userBehaviorMapper.selectByUserId(userId, 1000);
    if (CollUtil.isEmpty(behaviors)) return;
    
    // 2. 计算用户画像指标
    UserProfile profile = userProfileMapper.selectByUserId(userId);
    if (profile == null) {
        profile = new UserProfile();
        profile.setUserId(userId);
    }
    
    // 3. 计算各项指标
    calculateActivityLevel(profile, behaviors);      // 活跃度
    calculatePurchaseFrequency(profile, userId);     // 购买频率
    calculatePriceSensitivity(profile, behaviors);   // 价格敏感度
    calculateQualityRequirement(profile, behaviors); // 质量要求
    
    // 4. 保存用户画像
    saveOrUpdateProfile(profile);
}
```

### 4.2 画像指标计算
```java
// 活跃度计算 (基于最近30天行为)
private void calculateActivityLevel(UserProfile profile, List<UserBehavior> behaviors) {
    long recentBehaviors = behaviors.stream()
        .filter(b -> b.getBehaviorTime().isAfter(LocalDateTime.now().minusDays(30)))
        .count();
    profile.setActivityLevel(BigDecimal.valueOf(Math.min(recentBehaviors / 30.0, 1.0)));
}

// 价格敏感度计算 (基于购买商品价格)
private void calculatePriceSensitivity(UserProfile profile, List<UserBehavior> behaviors) {
    List<UserBehavior> purchaseBehaviors = behaviors.stream()
        .filter(b -> b.getBehaviorType() == 5)
        .collect(Collectors.toList());
    
    if (!purchaseBehaviors.isEmpty()) {
        double avgPurchasePrice = calculateAveragePurchasePrice(purchaseBehaviors);
        // 价格敏感度 = (最高价格 - 平均购买价格) / 最高价格
        double sensitivity = Math.max(0.0, Math.min(1.0, (200.0 - avgPurchasePrice) / 200.0));
        profile.setPriceSensitivity(BigDecimal.valueOf(sensitivity));
    } else {
        profile.setPriceSensitivity(BigDecimal.valueOf(0.5)); // 默认中等敏感度
    }
}
```

## 5. API接口设计

### 5.1 推荐接口
```java
@RestController
@RequestMapping("/wx/recommend")
public class WxRecommendController {
    
    @GetMapping("/list")
    public Result<List<RecommendationVO>> recommend(@RequestParam(defaultValue = "10") Integer limit) {
        Long userId = SecurityUtils.getCurrentUserId();
        List<RecommendationVO> list;
        
        if (userId == null) {
            // 未登录用户返回热门推荐
            list = recommendationService.popularRecommend(limit);
        } else {
            // 已登录用户返回混合推荐
            list = recommendationService.hybridRecommend(userId, limit);
        }
        return Result.success(list);
    }
    
    @PostMapping("/behavior")
    public Result<Void> reportBehavior(@Valid @RequestBody UserBehaviorReportDTO dto) {
        Long userId = SecurityUtils.getCurrentUserId();
        if (userId == null) {
            return Result.fail(ResultCode.USER_NOT_LOGIN);
        }
        
        recommendationService.recordUserBehavior(userId, dto.getBehaviorType(), 
            1, dto.getSeedId(), dto.getSource(), dto.getDuration().doubleValue());
        return Result.success();
    }
}
```

### 5.2 管理接口
```java
@RestController  
@RequestMapping("/recommendation")
public class RecommendationController {
    
    // 协同过滤推荐
    @GetMapping("/collaborative/{userId}")
    public Result<List<RecommendationVO>> collaborativeFilteringRecommend(
        @PathVariable Long userId, @RequestParam(defaultValue = "10") Integer limit) {
        List<RecommendationVO> recommendations = 
            recommendationService.collaborativeFilteringRecommend(userId, limit);
        return Result.success(recommendations);
    }
    
    // 手动更新用户画像
    @PostMapping("/profile/update/{userId}")
    public Result<Void> updateUserProfile(@PathVariable Long userId) {
        recommendationService.updateUserProfile(userId);
        return Result.success();
    }
    
    // 分页查询推荐结果
    @GetMapping("/page")
    public Result<PageResult<Recommendation>> getRecommendationPage(
        PageQuery pageQuery, @RequestParam(required = false) Long userId) {
        // 实现分页查询逻辑
        return Result.success(pageResult);
    }
}
```

## 10. 推荐模块实现详述

### 10.1 系统架构与设计理念

本推荐系统采用了分层架构设计，从底层数据收集到顶层算法融合，构建了一个完整的智能推荐生态。系统的核心设计理念是通过多维度数据挖掘和多算法协同工作，为用户提供精准、多样化的个性化推荐服务。在架构层面，系统分为数据层、算法层、服务层和应用层四个主要层次。数据层负责收集和存储用户行为数据、用户画像信息以及商品特征数据，为上层算法提供丰富的数据基础；算法层实现了协同过滤、内容推荐、个性化推荐和热门推荐四种核心算法，每种算法都有其独特的应用场景和优势；服务层通过Spring Boot框架提供RESTful API接口，实现了推荐服务的统一管理和调用；应用层则包括Web管理端和小程序用户端，为不同角色的用户提供相应的功能界面。

整个系统的设计充分考虑了电商场景的特殊性，特别是在处理用户重复购买行为、商品季节性特征以及新用户冷启动等问题上，采用了创新的解决方案。例如，在协同过滤算法中，系统并不简单地过滤用户已购买的商品，而是采用智能降权的策略，将最近30天内购买的商品权重降低到30%，这样既避免了过度推荐用户已拥有的商品，又保持了推荐的多样性和实用性。同时，系统引入了时间衰减机制，使得用户的历史行为随时间推移逐渐降低影响权重，确保推荐结果能够反映用户当前的兴趣偏好。

### 10.2 核心算法实现与优化

系统的核心竞争力在于其精心设计的多算法融合机制和先进的相似度计算方法。在协同过滤算法的实现中，系统采用了基于用户的协同过滤方法，通过计算用户之间的相似度来预测目标用户对未接触商品的偏好程度。相似度计算采用了创新的双重加权策略：首先通过余弦相似度计算用户行为向量的相似性，然后结合用户画像特征计算画像相似度，最终将行为相似度（权重70%）和画像相似度（权重30%）进行加权融合，得到更加准确和全面的用户相似度评分。这种方法不仅考虑了用户的显性行为模式，还融入了用户的隐性特征属性，显著提升了推荐的准确性。

在隐式评分机制的设计上，系统根据不同行为类型的重要性和可信度，为每种用户行为分配了差异化的基础评分：浏览行为1.0分、搜索行为1.5分、收藏行为3.0分、加购物车行为3.5分、购买行为5.0分、评价行为4.0分。这种评分体系充分体现了用户行为的价值层次，购买行为作为最强的偏好信号获得最高评分，而评价行为虽然表达了明确的态度但可能存在主观性因此评分略低于购买。系统还引入了行为权重和时间衰减因子的概念，使得评分计算更加精细化：行为权重反映了特定行为在特定上下文中的重要性，而时间衰减因子则确保了近期行为比历史行为具有更高的影响力，30天内的行为保持原始权重，之后按照指数衰减规律逐步降低权重。

### 10.3 用户画像建模与动态更新

用户画像系统是推荐引擎的重要组成部分，它通过对用户历史行为数据的深度挖掘和统计分析，构建了多维度的用户特征模型。系统设计了五个核心画像指标：活跃度反映用户在平台上的参与程度，通过统计用户最近30天的行为次数并标准化到0-1区间；购买频率体现用户的消费活跃度，通过历史购买次数与基准值的比较来量化；价格敏感度通过分析用户购买商品的价格分布特征来判断用户对价格的敏感程度，计算公式为(最高价格-平均购买价格)/最高价格；质量要求通过用户的评价行为和评分数据来衡量用户对商品质量的期望水平；品牌忠诚度则通过分析用户对特定品牌的重复购买行为来评估。这些指标的计算都基于严格的数学模型，确保了画像数据的准确性和可解释性。

用户画像的更新采用了异步机制，每当用户产生新的行为数据时，系统会触发异步任务来重新计算和更新用户画像。这种设计既保证了画像数据的实时性，又避免了同步更新对系统性能的影响。更新过程中，系统会综合考虑用户的全部历史行为，而不是简单的增量更新，这样可以确保画像的稳定性和一致性。同时，系统还实现了画像数据的版本管理和回滚机制，在出现异常情况时可以快速恢复到之前的稳定状态。

### 10.4 混合推荐策略与性能优化

混合推荐是系统的核心创新点，它将四种不同的推荐算法按照科学的权重比例进行融合：协同过滤推荐占30%权重，主要解决基于用户相似性的推荐问题；内容推荐占25%权重，通过商品特征匹配来发现用户可能感兴趣的商品；个性化推荐占25%权重，基于用户画像进行精准匹配；热门推荐占20%权重，确保推荐结果的多样性和流行度。这种权重分配经过了大量的实验验证和调优，既保证了推荐的个性化程度，又兼顾了推荐的覆盖面和新颖性。在算法融合过程中，系统采用了加权评分的方法，将每种算法的推荐结果按照其权重进行评分计算，然后通过归一化处理得到最终的推荐评分，确保了不同算法结果的公平竞争。

在性能优化方面，系统采用了多层次的优化策略。首先是算法层面的优化，通过相似度计算的并行化、评分矩阵的稀疏化处理以及推荐结果的缓存机制，显著提升了算法的执行效率。其次是数据层面的优化，通过合理的数据库索引设计、分区表的使用以及读写分离的架构，保证了大数据量下的查询性能。最后是系统架构层面的优化，采用了异步处理、消息队列、分布式缓存等技术，实现了系统的高并发和高可用。系统还建立了完善的监控和预警机制，实时监控推荐服务的性能指标，包括响应时间、吞吐量、准确率等关键指标，确保系统能够稳定运行并及时发现和解决潜在问题。通过这些综合性的优化措施，系统能够在保证推荐质量的前提下，实现毫秒级的响应速度和高并发的服务能力。

